...
// Initialize an array to hold the student's details for the departments where data is found
$studentDetails = null;

// Fetch the student details from the first department (assuming the student is in at least one department)
foreach ($departments as $dept) {
    ...
    if (!$student) {
        continue; // Skip this department if student data is not found
    }
    ...
    if (empty($programmes)) {
        continue; // Skip this department if programme enrolment is not found
    }
    ...
    // Add this department's student details to the array
    $studentDetails[] = [
        'student' => $student,
...
// Display the student details (only departments with data are included)
if (empty($studentDetails)) {
    die("❌ No data found for the student in any department.");
}
// Display the student details once
?>
<?php
// Student personal details (already fetched)
?>
                        ...
                        <div class="details-grid">
                           ...
                           <span><?= htmlspecialchars($student['first_name'] . ' ' . $student['last_name']) ?></span>
                            ...
                            <div class="detail-item">
                                ...
                                <span id="personal-email"><?php echo htmlspecialchars($student['personal_email']) ?></span>
                            </div>
                            <div class="detail-item">
                                <label>Address:</label>
                                <span id="address"><?php echo htmlspecialchars($student['full_address']) ?></span>
                            </div>
                            <div class="detail-item">
                                <label>University Email:</label>
                                <span id="university-email"><?php echo htmlspecialchars($student['uni_email']) ?></span>
                            </div>
                        </div>
                    </div>

<?php
// Loop over each department and display all in one loop
foreach ($studentDetails as $details):
    $programmes = $details['programmes'];
    $modulesTaken = $details['modulesTaken'];
    $currentModules = $details['currentModules'];
    $tuitionFeeDetails = $details['tuitionFeeDetails'];
        ...
        <table>
            <tbody>
                <?php foreach ($tuitionFeeDetails as $fee): ?>
                    ...
                    <!-- <tr><td><strong>Due Date:</strong></td><td><?= htmlspecialchars($fee['due_date']) ?></td></tr> -->
