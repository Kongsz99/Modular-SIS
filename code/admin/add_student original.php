...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $programmes_selected = [
        ['department' => $department1, 'programme' => $programme1, 'start_date' => $p1_start_date, 'current_year' => $p1_current_year],
    ];

    if ($department2) {
        $programmes_selected[] = ['department' => $department2, 'programme' => $programme2, 'start_date' => $p2_start_date, 'current_year' => $p2_current_year];
    }
    ...
    foreach ($programmes_selected as $programme) {
        ...
        $query = "INSERT INTO student_programme (
            student_id, programme_id, programme_start_date, current_year
        ) VALUES (?, ?, ?, ?)";

        $stmt = $target_db->prepare($query);
        $stmt->execute([
            $student_id, $programme['programme'], $programme['start_date'], $programme['current_year']
        ]);
    }
